...
def login_page():
    if 'username' in session:
        return redirect(url_for('inventory'))  # Redirect to inventory if already logged in
...
def dashboard():
    ...
    return redirect(url_for('inventory'))  # Redirect to inventory page after successful login


@app.route('/inventory')
def inventory():
    if 'username' not in session:
        return redirect(url_for('login_page'))  # Redirect to login if user is not authenticated
    return render_template('inventory.html')  # Serve the inventory page
...
def check_user():
    try:
        ...
        if user:
            ...
            return redirect(url_for('inventory'))  # Redirect to inventory after successful login
        else:
...
def manage_products():
    ...
    if request.method == 'POST':
        ...
        quantity = data.get('quantity', 0)  # Default to 0 if quantity is not provided
        ...
        try:
            ...
            product_list = [{'id': row[0], 'name': row[1], 'price': row[2], 'quantity': row[3]} for row in products]
...
def modify_product(product_id):
    ...
    if request.method == 'GET':
        try:
            ...
            if product:
                return jsonify({'id': product[0], 'name': product[1], 'price': product[2], 'quantity': product[3]})
            else:
